...
function List({ setEditData, filter, sort, errorMessage, setErrorMessage }) {
  ...
  const handleBlockAccount = async (accountId) => {
    try {
      await axios.put(
        `${URL}/bank/api/block/${accountId}`,
        {},
        { withCredentials: true }
      );
      console.log("Block account id: ", accountId);
  ...
  const handleUnblockAccount = async (accountId) => {
    try {
      await axios.put(
        `${URL}/bank/api/unblock/${accountId}`,
        {},
        { withCredentials: true }
      );
      console.log("Unblock account id: ", accountId);
        ...
        {filteredList.map((a) => (
          <div key={a.id} className="client">
            ...
            <div className="client__data">
              ...
              <span className="client__input-text">{a.surname}</span>
            </div>
            <div className="client__data">
              <span className="client__label-text">Balance:</span>{" "}
              <span className="client__input-text">{a.balance}</span>
            </div>
            <div className="btn btn-list">
              <button
                className={`btn block${
                  activeButtons[a.id] === "block" ? "btn block--active" : ""
                }`}
              ...
              <button
                className={`btn unblock${
                  activeButtons[a.id] === "unblock" ? "btn unblock--active" : ""
                }`}
              ...
              <div
                ...
                onClick={() =>
                  handleActionIfNotBlocked(a, () => handleDelete(a))
                }
              >
                Delete
              </div>
              <div
                className="btn edit"
                onClick={() =>
                  handleActionIfNotBlocked(a, () => setEditModal(a))
                }
              >
                Edit
              </div>
